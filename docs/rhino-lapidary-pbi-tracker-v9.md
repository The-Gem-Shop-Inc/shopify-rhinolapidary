...
|---:|---|---|:---:|:---:|---|---|
| 1 | A-028 | Create automated storefront smoke test suite | P0 | M | Guidance provided | Turns the fixture data into customer-journey tests that catch broken pages before manual review. |
| 2 | A-031 | Document cart and checkout-entry architecture | P0 | M | Guidance provided | Purchase-path behavior is launch-critical and should be verified by smoke tests. |
| 3 | A-032 | Verify cart type and cart notification behavior | P1 | S | Guidance provided | Current theme uses cart notification; this creates automated checks around that decision. |
| 4 | A-036 | Define Search and Discovery configuration process | P1 | M | Guidance provided | Connects the filter spec to Shopify-admin state and creates an auditable export/check process. |
| 5 | A-037 | Define metafield and metaobject architecture | P1 | M | Guidance provided | Establishes typed product data sources needed by filters, product templates, SEO, and support content. |
